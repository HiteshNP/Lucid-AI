...
def process_pdf(uploaded_file):
    ...
    my_progress = st.progress(0, text="Activating the LucidAI-Slide-Eradicator-inator...")
...
# List to store titles extracted from PDF
    ...
    while retry_count < max_retries and not success:
        ...
        for i in range(25, 80):
            if retry_count == 0:
                my_progress.progress(i, "Supercharging your presentation with LucidAI!")
                time.sleep(1)
            else:
...
st.set_page_config(
    page_title="LucidAI-Slide-Eradicator-inator",
...
st.title("🧙‍♂️ LucidAI-Slide-Eradicator-inator")
...
That's why we invented the **LucidAI-Slide-Eradicator-inator**!   This super-cool Gemini-powered tool uses some serious science to transform those cryptic slides into clear, understandable slides. ✨
...
uploaded_file = st.file_uploader("Step 1: Choose a LucidAI PDF file", type=['pdf'])
...
process_button = st.button("Process the PDF!")
